...
class _OpportunityRepository:
    """
    Plain CRUD, validating that location_id/contact_id/agreement_ids
    reference real records, plus triggering waitlist promotion on update()
    -- the second of the registration engine's two triggers (see
...
class _ParticipantRepository:
    """
    Owns the registration engine. Every mutating method here (and
    _OpportunityRepository.update()) locks the opportunity row first --
    `SELECT ... FOR UPDATE` -- before reading or writing any of its
    participants. Because that's the *only* lock ever taken, and it's
    always the same single row per transaction, there's no lock-ordering
    deadlock to worry about; holding it is what makes the capacity reads
    and FIFO promotion below safe under concurrent registrations.
    """
    ...
    async def register(self, user_id: int, opportunity_id: int) -> models.StoredParticipant:
        ...
        async with self._store.pool.acquire() as conn, conn.transaction():
            opportunity = await conn.fetchrow(
                "SELECT capacity FROM opportunities WHERE id = $1 FOR UPDATE", opportunity_id
            )
            ...
            active = await conn.fetchval(
                ...
                "WHERE user_id = $1 AND opportunity_id = $2 AND NOT cancelled",
                user_id,
                opportunity_id,
            )
            if active:
    ...
    async def cancel(self, participant_id: int) -> None:
        ...
        opportunity_id = await self._store.pool.fetchval(
            "SELECT opportunity_id FROM participants WHERE id = $1", participant_id
        )
        ...
        async with self._store.pool.acquire() as conn, conn.transaction():
            await conn.execute(
                "SELECT id FROM opportunities WHERE id = $1 FOR UPDATE", opportunity_id
            )
            participant = await conn.fetchrow(
                "SELECT approved, cancelled FROM participants WHERE id = $1", participant_id
            )
    ...
    async def _promote_waitlist(
        ...
        capacity = await conn.fetchval(
            "SELECT capacity FROM opportunities WHERE id = $1", opportunity_id
        )
        approved_count = await conn.fetchval(
            "SELECT count(*) FROM participants "
            "WHERE opportunity_id = $1 AND approved AND NOT cancelled",
